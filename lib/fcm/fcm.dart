...
class FCM {
  ...
  static Future<void> sendPushMessage(
      {required String token,required String body, required String title,Map<String, dynamic>? data})
  async {
    ...
    var response =  await http.post(
        Uri.parse(
            'https://fcm.googleapis.com/v1/projects/phishing-c4f43/messages:send'),
  ...
  static Future<void> sendPushMessageToMultipleUsers(
      {required List<String> tokens, required String body, required String title, Map<String, dynamic>? data}) async {
    var accessToken = await FirebaseAccessToken().generateFirebaseAccessToken();

    for (String token in tokens) {
      try {
        var response = await http.post(
          Uri.parse('https://fcm.googleapis.com/v1/projects/phishing-c4f43/messages:send'),
          headers: <String, String>{
            'Content-Type': 'application/json; charset=UTF-8',
            'Authorization': 'Bearer $accessToken',
          },
        ...
        log(response.body.toString());
        print('FCM request for device sent for token: $token');
      } catch (e) {
        log(token.toString());
        print('Error sending FCM message to token $token: $e');
      }
    }
  }
